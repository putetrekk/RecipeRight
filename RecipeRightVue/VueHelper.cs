...
namespace RecipeRightVue
{
    public static class VueHelper
    {
        ...
        private static List<string> DoneMessage { get; } = new List<string> {
            "DONE  Compiled successfully in",
            "WARNING  Compiled with",
            ...
            spa.UseProxyToSpaDevelopmentServer(async () =>
            {
                ...
                _ = Task.Run(() =>
                {
                    try
                    {
                        ...
                        while ((line = process.StandardOutput.ReadLine()) != null)
                        {
                            ...
                            if (!tcs.Task.IsCompleted && DoneMessage.Any(msg => line.Contains(msg)))
                            {
                                tcs.SetResult(1);
                            }
                        }
                ...
                _ = Task.Run(() =>
                {
                    try
                    {
                        ...
                        {
                            if (!line.Contains("[webpack.Progress]"))
